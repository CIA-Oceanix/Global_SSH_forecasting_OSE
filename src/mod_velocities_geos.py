...
def retreive_geos_velocities(maps, var_name, model_name):
    ...
    mdt_UV = xr.open_dataset("/Odyssey/public/duacs/cnes_obs-sl_glo_phy-mdt_my_0.125deg_P20Y_multi-vars_179.94W-179.94E_89.94S-89.94N_2003-01-01.nc")
    #.isel(time=0).expand_dims({'time': maps.time.values}, axis=0)
    mdt_UV["longitude"] = (mdt_UV["longitude"] % 360)
    #.where(mdt_UV["longitude"] != 360, 0)
    ...
    # Interpolate dataset
    maps = maps.interp(latitude = mdt_UV.latitude, longitude = mdt_UV.longitude)
    # BEFORE : (latitude=new_lat, longitude=new_lon, method="linear")
    ...
    ugos_cpu, vgos_cpu, ugos_cpu_a, vgos_cpu_a = compute_geostrophic_velocity(lat, lon, maps["sla"], MDT_u, MDT_v, var_name)
    ...
    maps["ugosa"] = (('time', 'latitude', 'longitude'), ugos_cpu_a)
    maps["vgosa"] = (('time', 'latitude', 'longitude'), vgos_cpu_a)
    
    maps["mdt_u"] = (('time', 'latitude', 'longitude'), MDT_u)
    maps["mdt_v"] = (('time', 'latitude', 'longitude'), MDT_v)
