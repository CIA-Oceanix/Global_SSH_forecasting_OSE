...
def interpolate_current_intervals(df, time_series, start, end, var='ssh', forecast_interval=False, **kwargs):
    """
    Interpolate the time series over the defined period.
...
def run_interpolation_drifters(ds_maps, ds_drifter, time_min, time_max, restricted, frequency='M'):
    """
    ...
    print(str(ds_drifter.TIME.values[0]).split('-')[0])
    if(str(ds_drifter.TIME.values[0]).split('-')[0] == '2023' or str(ds_drifter.TIME.values[0]).split('-')[0] == '2024'):
        df = ds_drifter.to_dataframe().reset_index().set_index("TIME")
        #df = df.rename({"LONGITUDE": "longitude", "LATITUDE": "latitude"})
        df = df.rename(columns={"LONGITUDE": "longitude", "LATITUDE": "latitude", "TIME": "time"})
        #ds_drifter.to_dataframe()
        for start, end in periods(df, time_series, var_name='NSCT', frequency=frequency):
            if(restricted):
                interpolate_current_intervals(df, time_series, start, end, var='ssh', forecast_interval=True)
            else:
