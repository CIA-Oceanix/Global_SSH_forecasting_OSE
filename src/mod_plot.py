...
def plot_benchmark_scorecard(data, figsize, var_param):
    import numpy as np
    import pandas as pd
    ...
    common_index = next(iter(data.values())).index
    rows_ssh = list(common_index)
    df_ssh = data['SLA nRMS (MDT Mercator and MDT DUACS)'].iloc[:,:3]
    df_ssh_ssh_related = data['SLA nRMS (MDT DUACS)'].iloc[:,:3]
    df_vel_aligned = data['% Vel Magnitude (MDT Mercator and MDT DUACS)'].iloc[:,:3]
    df_angles_aligned = data['% Vel Angles (MDT Mercator and MDT DUACS)'].iloc[:,:3]
    df_sep_distances_aligned = data['Sep.Distance'].iloc[:,:3]

    # --------------------------
    # Align indices & columns for merging
    # --------------------------

    df_combined = pd.concat([df_ssh, df_ssh_ssh_related, df_vel_aligned, df_angles_aligned, df_sep_distances_aligned], axis=1)

    baseline = df_combined.loc["GLO12"]

    percent_diff = (df_combined - baseline) / baseline * 100

    plt.rcParams['figure.facecolor'] = '0.9'

    reds = sns.color_palette('Reds_r', 6)
    blues = sns.color_palette('Blues', 6)
    cmap = mpl.colors.ListedColormap(reds + [(0.95, 0.95, 0.95)] + blues)

    cb_levels = [-35, -20, -10, -5, -1, 1, 5, 10, 15]
    norm = mpl.colors.BoundaryNorm(cb_levels, cmap.N, extend='both')

    nmodels = len(df_combined)
    nvariables = len(df_combined.columns)

    fig_height = 0.4 * nmodels + 2
    if figsize is None:
        fig_height = 0.4 * nmodels + 2
        fig_width = 1.2 * nvariables + var_param
        figsize = (fig_width, fig_height)
    
    fig, ax = plt.subplots(figsize=figsize)

    img = ax.imshow(percent_diff.values, cmap=cmap, norm=norm, aspect='auto')

    # Set minor ticks at cell edges for gridlines
    ax.set_xticks(np.arange(-0.5, nvariables, 1), minor=True)
    ax.set_yticks(np.arange(-0.5, nmodels, 1), minor=True)

    # Draw white gridlines for cell borders
    ax.grid(which='minor', color='white', linestyle='-', linewidth=1)

    # Hide minor tick markers
    ax.tick_params(which='minor', bottom=False, left=False)

    # Add vertical white gaps between groups
    column_split_indices = [2, 5, 8, 11, 14]  # After 3rd and 6th columns
    '''
    for split_idx in column_split_indices:
        ax.add_patch(patches.Rectangle(
            (split_idx + 0.5, -0.5), 0.1, nmodels,
            facecolor='white', edgecolor='none',
            zorder=10
        ))
    '''

    # Add black vertical border lines along the white gap edges to keep borders visible
    '''
    for split_idx in column_split_indices:
        ax.axvline(split_idx + 0.5, color='black', linewidth=1, zorder=11)
        ax.axvline(split_idx - 0.5, color='black', linewidth=1, zorder=11)
    '''

    # Add black border around the entire heatmap
    '''
    ax.axhline(-0.5, color='black', linewidth=1, zorder=11)             # top border
    ax.axhline(nmodels - 0.5, color='black', linewidth=1, zorder=11)   # bottom border
    ax.axvline(-0.5, color='black', linewidth=1, zorder=11)            # left border
    ax.axvline(nvariables - 0.5, color='black', linewidth=1, zorder=11) # right border
    '''

    # Axis labels and ticks — only label 0, 3, and 5
    xticks = np.arange(nvariables)
    xticklabels = [str(col.split("(")[-1].replace(" days)", "").replace(" day)", ""))
                if any(day in col for day in ["0 day", "3 days", "5 days"]) else ""
                for col in df_combined.columns]

    ax.set_xticks(xticks)
    ax.set_xticklabels(xticklabels, rotation=0, ha='center', fontsize=9)
    ax.set_yticks(np.arange(nmodels))
    ax.set_yticklabels(df_combined.index, fontsize=9)

    # Annotate values (only if not NaN)
    for i in range(nmodels):
        for j in range(nvariables):
            val = df_combined.iloc[i, j]
            if pd.notna(val):
                text = f"{val:.3f}"
                ax.text(j, i, text, ha='center', va='center', fontsize=7)

    # Highlight DUACS row
    duacs_idx = df_combined.index.get_loc("Duacs")
    ax.add_patch(patches.Rectangle(
        (-0.5, duacs_idx - 0.5),
        width=nvariables, height=1,
        linewidth=2,
        edgecolor='black',
        facecolor='none'
    ))

    # Group color highlights (same groups as original)
    group_colors = {
        "Model-based": '#f0f0f0',
        "Emulators": '#e0f7fa',
        "Observation-based": '#fce4ec',
    }

    group_bounds = {
        "Model-based": (0, 1),
        "Emulators": (1, 3),
        "Observation-based": (3, 12),
    }

    for group, (start, end) in group_bounds.items():
        ax.add_patch(patches.Rectangle(
            (-3.7, start - 0.5), 3.2, end - start,
    ...
    # Vertical group labels8215629837275469
    group_labels = {
        "Model-based \n forecasts": (0, 2),
        "Neural Emulators": (2, 6),
        "Observation-based \n neural forecasts": (6, 14),
    }
    for label, (start, end) in group_labels.items():
        mid = (start + end - 1) / 2
        ax.text(-2., mid, label, va='center', ha='center',
                fontsize=10, rotation=0, color='black',
                transform=ax.transData, style='oblique')

    # Colorbar
    cbar = fig.colorbar(img, ax=ax, orientation='horizontal', pad=0.08, fraction=0.05)
    cbar.set_label("% Difference to GLO12 SLA")
    cbar.set_ticks(cb_levels)
    cbar.ax.set_xlabel(r'Worse $\longleftarrow$     % Difference in Score     $\longrightarrow$ Better', fontsize=10, labelpad=10)

    # Column group labels
    col_group_labels = [
        ("nRMSE score, SLA related", 0, 3),
        ("nRMSE score, SSH related", 3, 3),
        ("% Vel. Magnitude, SLA related", 6, 3),
        ("% Vel. Angles, SLA related", 9, 3),
        ("Sep. Distance", 12, 3),
    ]

    for label, start, width in col_group_labels:
        mid = start + width / 2 - 0.5
        ax.text(mid, -1., label, ha='center', va='bottom',
...
def plot_single_table_seperate_colorbars(df_combined, title_suffix, col_group_labels, fig_id, cb_levels_dict):
    import numpy as np
    import pandas as pd
    import seaborn as sns
    import matplotlib.pyplot as plt
    ...
    fig_height = 0.4 * nmodels + 2
    fig_width = 1. * nvariables + 2
    figsize = (fig_width, fig_height)

    fig, ax = plt.subplots(figsize=figsize)

    # Loop over each cell and manually color it
    for i in range(nmodels):
        for j in range(nvariables):
            val = percent_diff.iloc[i, j]
            if pd.isna(val):
                color = (0.95, 0.95, 0.95)
            else:
                # Find which group the column belongs to
                for label, start, width in col_group_labels:
                    if start <= j < start + width:
                        levels = cb_levels_dict[label]
                        reds = sns.color_palette('Reds_r', 6)
                        blues = sns.color_palette('Blues', 6)
                        cmap = mpl.colors.ListedColormap(reds + [(0.95, 0.95, 0.95)] + blues)
                        ...
                        color = cmap(norm(val))
                        break
            rect = patches.Rectangle((j - 0.5, i - 0.5), 1, 1, facecolor=color, edgecolor='white')
            ax.add_patch(rect)

    # Grid and ticks
    ax.set_xlim(-0.5, nvariables - 0.5)
    ax.set_ylim(nmodels - 0.5, -0.5)
    ax.set_xticks(np.arange(nvariables))
    ax.set_xticklabels(
        [str(col.split("(")[-1].replace(" days)", "").replace(" day)", "")) if "day" in col else "" for col in df_combined.columns],
    ...
    # DUACS highlight
    if "Duacs" in df_combined.index:
        duacs_idx = df_combined.index.get_loc("Duacs")
        ax.add_patch(patches.Rectangle((-0.5, duacs_idx - 0.5), nvariables, 1,
                                       linewidth=2, edgecolor='black', facecolor='none'))

    # Background group highlighting (row blocks)
    group_colors = {
        "Model-based": '#f0f0f0',
        "Emulators": '#e0f7fa',
        "Observation-based": '#fce4ec',
    ...
    group_bounds = {
        "Model-based": (0, 1),
        "Emulators": (1, 4),
        "Observation-based": (4, nmodels - 1),
    }
    for group, (start, end) in group_bounds.items():
        ax.add_patch(patches.Rectangle(
            (-3.3, start - 0.5), 4.8, end - start,
            facecolor=group_colors[group], edgecolor='none',
            transform=ax.transData, clip_on=False, zorder=0
        ))

    group_labels = {
        "Model-based \n forecasts": (0, 1),
        "Neural Emulators": (1, 6),
        "Observation-based \n neural forecasts": (6, 14),
    }
    '''for label, (start, end) in group_labels.items():
        if(label == "Observation-based \n neural forecasts"):
            mid = (start + end - 13.)/2
        else:
            mid = (start + end - 1.1) / 2.8
        ax.text(-2.5, mid, label, va='center', ha='center',
                fontsize=10, rotation=0, color='black', transform=ax.transData, style='oblique')'''
    ...
    # Custom colorbars for each group
    for idx, (label, start, width) in enumerate(col_group_labels):
        levels = cb_levels_dict[label]
        reds = sns.color_palette('Reds_r', 6)
        blues = sns.color_palette('Blues', 6)
        cmap = mpl.colors.ListedColormap(reds + [(0.95, 0.95, 0.95)] + blues)
        norm = mpl.colors.BoundaryNorm(levels, cmap.N, extend='both')
        cax = fig.add_axes([0.22 + 0.2 * idx, 0.08, 0.15, 0.015])
        cb = mpl.colorbar.ColorbarBase(cax, cmap=cmap, norm=norm, orientation='horizontal')
        cb.set_label(label, fontsize=8)

    # Add background color per column group
    group_bg_colors = ['#f2f2f2', '#e6f7ff', '#fff0f5', '#e8f5e9']
    for idx, (label, start, width) in enumerate(col_group_labels):
        ax.add_patch(patches.Rectangle(
            (start - 0.5, -0.5),
            width, nmodels,
            facecolor=group_bg_colors[idx % len(group_bg_colors)],
            edgecolor='none',
            zorder=0
        ))

    # Add vertical separator lines between groups
    for _, start, width in col_group_labels[1:]:
...
def plot_single_table(df_combined, title_suffix, col_group_labels, fig_id, cb_levels):
        nmodels = len(df_combined)
        nvariables = len(df_combined.columns)
        ...
        fig_width = 1.2 * nvariables + var_param
        figsize = (fig_width, fig_height)
        #figsize or (fig_width, fig_height)
        ...
        for label, (start, end) in group_labels.items():
            mid = (start + end - 1) / 2
            ax.text(-1.6, mid, label, va='center', ha='center',
                    fontsize=10, rotation=0, color='black', transform=ax.transData, style='oblique')
        ...
        cbar = fig.colorbar(img, ax=ax, orientation='horizontal', pad=0.08, fraction=0.05)
        cbar.set_label(f"% Difference to GLO12 SLA – {title_suffix}")
        cbar.set_ticks(cb_levels)
        cbar.ax.set_xlabel(r'Worse $\longleftarrow$     % Difference in Score     $\longrightarrow$ Better', fontsize=10, labelpad=10)
...
def plot_benchmark_scorecard_split(data, figsize=None, var_param=2):
    # Load relevant subsets
    df_ssh = data['SLA nRMS (MDT Mercator and MDT DUACS)'].iloc[:, :3]
    df_ssh_ssh_related = data['SLA nRMS (MDT DUACS)'].iloc[:, :3]
    df_vel_aligned = data['% Vel Magnitude (MDT Mercator and MDT DUACS)'].iloc[:, :3]
    df_angles_aligned = data['% Vel Angles (MDT Mercator and MDT DUACS)'].iloc[:, :3]
    df_vel_aligned_ssh_related = data['% Vel Magnitude (MDT DUACS)'].iloc[:, :3]
    df_angles_aligned_ssh_related = data['% Vel Angles (MDT DUACS)'].iloc[:, :3]
    df_sep_distances_aligned = data['Sep.Distance'].iloc[:, :3]
    # Table 1: SSH + Vel Magnitude
    df_combined_1 = pd.concat([df_ssh, df_vel_aligned, df_angles_aligned, df_sep_distances_aligned], axis=1)
    col_group_labels_1 = [
        ("nRMSE score, MDT Mercator", 0, 3),
        ("% Vel. Magnitude, MDT Mercator", 3, 3),
        ("% Vel. Angles, MDT Mercator", 6, 3),
        ("Sep. Distance MDT Mercator", 9, 3),
    ]
    
    cb_levels_dict = {
        "nRMSE score, MDT Mercator": [-4, -3, -2, -1, 1, 2, 3, 4],
        "% Vel. Magnitude, MDT Mercator": [-12, -10, -2, -1, 1, 2, 10, 12],
        "% Vel. Angles, MDT Mercator": [-20, -15, -5, -1, 1, 5, 15, 20],
        "Sep. Distance MDT Mercator": [-10, -5, -2, -1, 1, 2, 5, 10],
    }

    plot_single_table_seperate_colorbars(df_combined_1, "Table 1", col_group_labels_1, 1, cb_levels_dict)

    # Table 2: Vel Angles + Sep Distances
    df_combined_2 = pd.concat([df_ssh_ssh_related, df_vel_aligned_ssh_related, df_angles_aligned_ssh_related, df_sep_distances_aligned], axis=1)
    col_group_labels_2 = [
        ("nRMSE score, MDT DUACS", 0, 3),
        ("% Vel. Magnitude, MDT DUACS", 3, 3),
        ("% Vel. Angles, MDT DUACS", 6, 3),
        ("Sep. Distance MDT DUACS", 9, 3),
    ]
    cb_levels_dict = {
            "nRMSE score, MDT DUACS": [-15, -14, -13, -11, 11,  13, 14, 15],
            "% Vel. Magnitude, MDT DUACS": [-12, -10, -2, -1, 1, 2, 10, 12],
            "% Vel. Angles, MDT DUACS": [-20, -15, -5, -1, 1, 5, 15, 20],
            "Sep. Distance MDT DUACS": [-10, -5, -2, -1, 1, 2, 5, 10],
        }    
    plot_single_table_seperate_colorbars(df_combined_2, "Table 2", col_group_labels_2, 2, cb_levels_dict)
...
def plot_stat_score_map(filename):
    ...
    # Define a finer regular grid
    lon_fine = np.linspace(0, 360,360*2)
    lat_fine = np.linspace(-90, 90, 180*2)

    # Interpolate the entire dataset (all variables) onto the fine grid
    ds_binning_allscale = ds_binning_allscale.interp(lon=lon_fine, lat=lat_fine, method='linear')
    ds_binning_filtered = ds_binning_filtered.interp(lon=lon_fine, lat=lat_fine, method='linear')
...
def plot_stats_lagrangian(sep_distances, times_sim, leadtime, model_name, filename_excel_stats):
    # === Analysis ===
    valid_time_indices = np.where(~np.isnan(sep_distances).any(axis=0))[0]
    max_time_idx = valid_time_indices[-1] + 1 if valid_time_indices.size > 0 else sep_distances.shape[1]
    sep_distances = sep_distances[:, :max_time_idx]

    # Cumulative and mean separation
    cum_sep_dist = np.nancumsum(sep_distances, axis=1)
    mean_cum_sep = np.nanmean(cum_sep_dist, axis=0)
    mean_sep = np.nanmean(sep_distances, axis=0)

    # Liu index
    total_disp = np.nanmax(sep_distances, axis=1)
    mean_total_disp = np.nanmean(total_disp)
    mean_inst_sep = np.nanmean(sep_distances)
    liu_index = mean_inst_sep / mean_total_disp if mean_total_disp > 0 else np.nan

    print('============ Leadtime ' + str(leadtime) + "============")
    print(f"Mean instantaneous separation distance (km): {mean_inst_sep/1e3:.2f}")
    print(f"Mean total displacement (max separation) (km): {mean_total_disp/1e3:.2f}")
    print(f"Liu index: {liu_index:.3f}")
    
    data = pd.read_excel(filename_excel_stats, engine="odf", sheet_name=None, index_col = 0)
    # Pick a reference sheet to copy structure (same index and columns)
    reference_sheet = data['SLA nRMS (MDT DUACS)']

    # Create a new empty DataFrame with same index and columns
    sep_distance_df = pd.DataFrame(index=reference_sheet.index, columns=reference_sheet.columns)

    # Add it as a new sheet
    data['Sep.Distance'] = sep_distance_df

    # Now you can assign values like this:
    model_name = model_name
    leadtime = 'Leadtime ' + str(leadtime)
    data['Sep.Distance'].loc[model_name, leadtime] = mean_total_disp / 1e3  # convert to km

    # Save to Excel
    with pd.ExcelWriter(filename_excel_stats, engine='odf') as writer:
        for sheet_name, df in data.items():
            df.to_excel(writer, sheet_name=sheet_name)
    
    print('Lagrangian stats are saved to ' + filename_excel_stats)

    # === Plot ===
    plt.figure(figsize=(10,6))
    dt_secs = np.median(np.diff(times_sim[0, :max_time_idx]).astype('timedelta64[s]').astype(int))
    time_axis_days = np.arange(max_time_idx) * dt_secs / 86400

    plt.plot(time_axis_days, mean_sep/1e3, label='Mean separation (km)')
    plt.xlabel('Days')
    plt.ylabel('Mean separation distance (km), leadtime ' + str(leadtime))
    plt.title('Mean separation distance between original and simulated drifters, leadtime ' + str(leadtime))
    plt.grid()
    plt.legend()
    plt.show()

def only_one_plot_stat_score_map(filename):
    """
    Generate and display error and explained variance maps.
...
def plot_stat_uv_by_regimes(stat_output_filename):
    ...
    for region in ['coastal', 'offshore_highvar', 'offshore_lowvar', 'equatorial_band', 'arctic', 'antarctic']:
        ...
        for var_name in ['mapping_err_u', 'mapping_err_v', 'ugos_interpolated', 'EWCT', 'vgos_interpolated', 'NSCT']:
            ...
            my_dictionary[f'{region}'][f'{var_name}_rms'] =  ds['rmse'].values[0]
            my_dictionary[f'{region}'][f'{var_name}_rms_score'] =  ds['rmse_score'].values[0]
